...
function sendStatusUpdateEmail($email, $name, $order_id, $status, $scheduled_date = null, $scheduled_time = null)
{
    ...
    $info = ($scheduled_date && $scheduled_time) ? "<p><strong>Scheduled Delivery:</strong> $scheduled_date at $scheduled_time</p>" : '';
...
function sendDelayNotificationEmail($email, $name, $order_id, $additional_time)
{
    ...
    $body = "<html><body><h2>Hello, $name</h2><p>Your order #$order_id may be delayed by an additional $additional_time hour(s).</p><p>Please let us know if this is acceptable.</p></body></html>";
    sendEmail($email, $name, $sub, $body);
}
function notifyDeliveryPerson($email, $name, $order_id, $status)
{
    $sub = "New Order Assigned - Order #$order_id";
    $body = "<html><body><h2>Hello, $name</h2><p>You have been assigned to order #$order_id with status $status.</p><p>Please proceed as required.</p></body></html>";
    sendEmail($email, $name, $sub, $body);
}
...
function getAllOrders($pdo)
{
    ...
    SELECT 
      ...
      o.delivery_user_id,o.status,o.is_deleted,t.name AS tip_name,t.percentage AS tip_percentage,t.amount AS tip_fixed_amount
    FROM orders o
    ...
    foreach ($orders as $o) {
        ...
        if ($d && isset($d['items']) && is_array($d['items'])) {
            foreach ($d['items'] as $item) {
                ...
                if ($q > 0) $counts[$name] = ($counts[$name] ?? 0) + $q;
            }
        }
    }
    arsort($counts);
    ...
    default:
        try {
            ...
            foreach ($all_orders as $o) {
                if (in_array($o['status'], $statuses)) $status_orders[$o['status']][] = $o;
                else $status_orders['New Order'][] = $o;
            }
    ...
    <div class="container-fluid">
        <nav class="mb-4">
            <ul class="nav nav-tabs">
                <li class="nav-item"><a class="nav-link<?= ($action !== 'view_trash' && $action !== 'top_products') ? ' active' : '' ?>" href="orders.php?action=view">View Orders</a></li>
                <?php if ($user_role === 'admin'): ?>
                    <li class="nav-item"><a class="nav-link<?= ($action === 'view_trash') ? ' active' : '' ?>" href="orders.php?action=view_trash">Trash</a></li>
                    <li class="nav-item"><a class="nav-link<?= ($action === 'top_products') ? ' active' : '' ?>" href="orders.php?action=top_products">Top 10 Products</a></li>
                <?php endif; ?>
            </ul>
        </nav>
                            ...
                            <div class="col-md-6">
                                ...
                                <p><strong>Tip:</strong>
                                    <?php if (!empty($order['tip_name'])) {
                                        $tip_display = $order['tip_percentage'] !== null ? htmlspecialchars($order['tip_percentage']) . '%' : number_format($order['tip_fixed_amount'], 2) . '€';
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" id="status" class="form-select" required>
                                    ...
                                    <?php foreach ($statuses as $so): ?>
                                        <option value="<?= htmlspecialchars($so) ?>" <?= ($order['status'] === $so) ? 'selected' : '' ?>><?= htmlspecialchars($so) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                                        ...
                                        <?php foreach ($trash_orders as $o): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                                    if (!empty($o['tip_name'])) {
                                                        $d = $o['tip_percentage'] !== null ? htmlspecialchars($o['tip_percentage']) . '%' : number_format($o['tip_fixed_amount'], 2) . '€';
                                                        echo "<span class='badge bg-info' data-bs-toggle='tooltip' title='Selected Tip: $d'>" . htmlspecialchars($o['tip_name']) . "</span>";
                                                    } else echo "N/A";
                                                    ?>
                                                </td>
                                                ...
                                                <td>
                                                    <a href="orders.php?action=restore&id=<?= $o['id'] ?>" class="btn btn-sm btn-success me-1" data-bs-toggle="tooltip" title="Restore Order"><i class="bi bi-arrow-counterclockwise"></i></a>
                                                    <a href="orders.php?action=permanent_delete&id=<?= $o['id'] ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to permanently delete this order? This action cannot be undone.');" data-bs-toggle="tooltip" title="Permanently Delete Order"><i class="bi bi-trash3-fill"></i></a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                ...
                foreach ($statuses as $st): ?>
                    <div class="status-section">
                                ...
                                <table class="table table-striped table-hover table-bordered align-middle data-table">
                                    <thead class="table-dark">
                                        <tr>
                                            ...
                                            <th>Created At</th>
                                            <th>Status</th>
                                            <?php if ($user_role === 'admin'): ?><th>Assign Delivery Person</th><?php endif; ?>
                                            <th>Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <?php foreach ($status_orders[$st] as $o) {
                                            ...
                                            if (!empty($o['tip_name'])) {
                                                $d = $o['tip_percentage'] !== null ? htmlspecialchars($o['tip_percentage']) . '%' : number_format($o['tip_fixed_amount'], 2) . '€';
                                                $ti = "<span class='badge bg-info' data-bs-toggle='tooltip' title='Selected Tip: $d'>" . htmlspecialchars($o['tip_name']) . "</span>";
                                            }
                                            echo "<tr>
                        <td>" . htmlspecialchars($o['id']) . "</td>
                        <td>" . htmlspecialchars($o['customer_name']) . "</td>
                            ...
                            <select name='delivery_user_id' class='form-select form-select-sm me-2' required>
                              <option value=''>Assign</option>";
                                                foreach ($delivery_users as $du) {
                                                    echo "<option value='" . htmlspecialchars($du['id']) . "'" . ($o['delivery_user_id'] == $du['id'] ? ' selected' : '') . ">" . htmlspecialchars($du['username']) . "</option>";
                                                }
                                                echo "</select><button type='submit' class='btn btn-sm btn-primary'>Assign</button></form></td>";
                                            }
                                            echo "<td>
                        ...
                        <a href='orders.php?action=update_status_form&id=" . $o['id'] . "' class='btn btn-sm btn-warning me-1' data-bs-toggle='tooltip' title='Update Status'><i class='bi bi-pencil'></i></a>";
                                            if ($user_role === 'admin') {
                                                echo "<a href='orders.php?action=delete&id=" . $o['id'] . "' class='btn btn-sm btn-danger me-1' onclick='return confirm(\"Are you sure you want to move this order to Trash?\");' data-bs-toggle='tooltip' title='Move to Trash'><i class='bi bi-trash'></i></a>
        ...
        $(document).ready(function() {
            ...
            function fetchNewOrders() {
                ...
                <tr>
                  ...
                  <td>${escapeHtml(o.customer_phone)}</td><td>${escapeHtml(o.delivery_address)}</td>
                  <td>${parseFloat(o.total_amount).toFixed(2)}€</td>
                  <td>${o.tip_id?'Tip Badge or Info':'N/A'}</td><td>${parseFloat(o.tip_amount).toFixed(2)}€</td>
                  <td>${escapeHtml(o.scheduled_date||'N/A')}</td><td>${escapeHtml(o.scheduled_time||'N/A')}</td>
                  <td>${escapeHtml(o.created_at)}</td><td>${escapeHtml(o.status)}</td>
                </tr>`;
                                    body.prepend(row);
                                });
                            }
