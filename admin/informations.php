...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'add_entry') {
    $type = $_POST['type'] ?? '';
    $day_of_week = ($type === 'regular') ? $_POST['day_of_week'] ?? NULL : NULL;
    $date = ($type === 'holiday') ? $_POST['date'] ?? NULL : NULL;
    ...
    $open_time = ($type === 'regular' || ($type === 'holiday' && !$is_closed)) ? ($_POST['open_time'] ?? NULL) : NULL;
    $close_time = ($type === 'regular' || ($type === 'holiday' && !$is_closed)) ? ($_POST['close_time'] ?? NULL) : NULL;
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            $error_message = "Failed to add entry.";
        }
    } else {
...
// Handle Edit Entry
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'edit_entry') {
    $id = $_POST['entry_id'] ?? 0;
    $type = $_POST['type'] ?? '';
    $day_of_week = ($type === 'regular') ? $_POST['day_of_week'] ?? NULL : NULL;
    $date = ($type === 'holiday') ? $_POST['date'] ?? NULL : NULL;
...
<body>
    <div class="container my-5">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h2 class="h5 mb-0">All Entries</h2>
                <button class="btn btn-success btn-sm" data-bs-toggle="modal" data-bs-target="#addEntryModal">Add Entry</button>
            </div>
                        ...
                        <tbody>
                            <?php foreach ($entries as $entry): ?>
                                <tr>
                                    ...
                                    <td><?= $entry['type'] === 'regular' ? htmlspecialchars($entry['day_of_week']) : htmlspecialchars($entry['date']) ?></td>
                                    ...
                                    <td>
                                        <button class="btn btn-primary btn-sm me-1" data-bs-toggle="modal" data-bs-target="#editEntryModal<?= $entry['id'] ?>">Edit</button>
                                        <button type="button" class="btn btn-danger btn-sm btn-delete" data-id="<?= htmlspecialchars($entry['id']) ?>">Delete</button>
                                    </td>
                                </tr>
                                            ...
                                            <form method="POST" action="informations.php">
                                                ...
                                                <div class="modal-body p-2">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select class="form-select form-select-sm" id="type<?= $entry['id'] ?>" name="type" onchange="toggleFormFields(this, <?= $entry['id'] ?>)" required>
                                                    ...
                                                    <div id="regularFields<?= $entry['id'] ?>" style="display: <?= $entry['type'] === 'regular' ? 'block' : 'none' ?>;">
                                                        <div class="mb-3">
                                                            ...
                                                            <select class="form-select form-select-sm" id="day_of_week<?= $entry['id'] ?>" name="day_of_week">
                                                    ...
                                                    <div id="holidayFields<?= $entry['id'] ?>" style="display: <?= $entry['type'] === 'holiday' ? 'block' : 'none' ?>;">
                                                        <div class="mb-3">
                                                            ...
                                                            <input type="date" class="form-control form-control-sm" id="date<?= $entry['id'] ?>" name="date" value="<?= htmlspecialchars($entry['date']) ?>">
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="title<?= $entry['id'] ?>" class="form-label">Title</label>
                                                            <input type="text" class="form-control form-control-sm" id="title<?= $entry['id'] ?>" name="title" value="<?= htmlspecialchars($entry['title']) ?>">
                                                        </div>
                                                        <div class="mb-3">
                                                            <label for="description<?= $entry['id'] ?>" class="form-label">Description</label>
                                                            <textarea class="form-control form-control-sm" id="description<?= $entry['id'] ?>" name="description" rows="2"><?= htmlspecialchars($entry['description']) ?></textarea>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="row mb-3">
                                                        <div class="col-md-6">
                                                            ...
                                                            <input type="time" class="form-control form-control-sm" id="open_time<?= $entry['id'] ?>" name="open_time" value="<?= htmlspecialchars($entry['open_time']) ?>" <?= $entry['is_closed'] ? 'disabled' : '' ?>>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label for="close_time<?= $entry['id'] ?>" class="form-label">Close Time</label>
                                                            <input type="time" class="form-control form-control-sm" id="close_time<?= $entry['id'] ?>" name="close_time" value="<?= htmlspecialchars($entry['close_time']) ?>" <?= $entry['is_closed'] ? 'disabled' : '' ?>>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="modal-footer p-2">
                                                    <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                                                    <button type="submit" class="btn btn-primary btn-sm">Update Entry</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                    ...
                    <form method="POST" action="informations.php">
                        ...
                        <div class="modal-body p-2">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select form-select-sm" id="type" name="type" onchange="toggleAddFormFields(this)" required>
                            ...
                            <div id="addHolidayFields" style="display: none;">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="title" name="title" placeholder="e.g., Christmas Day">
                                </div>
                                <div class="mb-3">
                                    <label for="description" class="form-label">Description</label>
                                    <textarea class="form-control form-control-sm" id="description" name="description" rows="2" placeholder="Details about the holiday"></textarea>
                                </div>
                            </div>
                            <div class="form-check mb-3">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="time" class="form-control form-control-sm" id="open_time" name="open_time">
                                </div>
                                <div class="col-md-6">
    ...
    <script>
        ...
        // Toggle Open/Close Time Fields in Add Entry
        ...
        // Toggle Open/Close Time Fields in Edit Entry
        ...
        // Toggle Form Fields in Edit Entry
