        ...
        if (!$message) {
            ...
            if (!$message) {
                ...
                try {
                    if ($editing) {
                        $sql = 'UPDATE products
                                SET product_code=?,category_id=?,name=?,description=?,allergies=?,
                                    image_url=?,is_new=?,is_offer=?,is_active=?,properties=?
                                WHERE id=?';
                        $params = [
                            $product_code,
                            $category_id,
                        ...
                        $sql = 'INSERT INTO products
                               (product_code,category_id,name,description,allergies,image_url,
                                is_new,is_offer,is_active,properties)
                                VALUES (?,?,?,?,?,?,?,?,?,?)';
                        $params = [
                            $product_code,
                            $category_id,
...
$props = ($editing && $product['properties'])
    ? json_decode($product['properties'], true)
    : [];

$has_extras_sauces = (!empty($props['extras'])
    || !empty($props['sauces'])
    || isset($_POST['has_extras_sauces']));

$has_sizes   = (!empty($props['sizes'])  || isset($_POST['has_sizes']));
$has_dresses = (!empty($props['dresses']) || isset($_POST['has_dresses']));

$base_price = isset($props['base_price'])
    ? $props['base_price']
...
$image_source = $editing
    ? (isset($_POST['image_source'])
        ...
        : ((!empty($product['image_url'])
            && strpos($product['image_url'], UPLOAD_DIR) === false)
            ? 'url' : 'upload'))
...
<div class="container mt-5">
    ...
    ?>
        <form method="POST" enctype="multipart/form-data" action="products.php?action=<?= $editing ? 'edit&id=' . $id : 'add' ?>">
            ...
            <div class="row g-3 mt-3">
                ...
                <div class="col-md-8">
                    <div class="form-check form-check-inline mt-2">
                        <input class="form-check-input" type="checkbox" name="has_extras_sauces"
                            <?= $has_extras_sauces ? 'checked' : '' ?>>
                        <label class="form-check-label">Has Extras & Sauces</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" name="has_sizes"
                            <?= $has_sizes ? 'checked' : '' ?>>
                        <label class="form-check-label">Has Sizes</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" name="has_dresses"
                            <?= $has_dresses ? 'checked' : '' ?>>
                        <label class="form-check-label">Has Dresses</label>
                    </div>

                    <div id="basePriceField" class="mt-3"
                        style="display:<?= $has_sizes ? 'none' : 'block' ?>;">
                    ...
                    <div id="extrasSaucesFields" class="mt-3"
                        style="display:<?= $has_extras_sauces ? 'block' : 'none' ?>;">
                        <label>Global Extras & Sauces</label>
                        <small class="text-muted d-block">
                            Define global extras and sauces for all sizes or base product.
                        </small>
                        <div class="row mt-2">
                            <div class="col-md-6">
                                        ...
                                        foreach ($props['extras'] as $ei => $ev) {
                                            ...
                                            <div class="col">
                                              <input type="number" step="0.01"
                                                     class="form-control"
                                                     name="global_extras[' . $ei . '][price]"
                                                     value="' . s($ev['price']) . '"
                                                     placeholder="Price">
                                            </div>
                                            <div class="col-auto">
                                              <button type="button"
                                                      class="btn btn-danger remove-global-extra">
                                                &times;
                                              </button>
                                            </div>
                                          </div>';
                                ...
                                <button type="button" class="btn btn-sm btn-primary mt-2" id="addGlobalExtra">
                                    Add Extra
                                </button>
                            </div>
                            ...
                            <div class="col-md-6">
                                    ...
                                    if (($editing || $adding) && isset($props['sauces'])) {
                                        foreach ($props['sauces'] as $si => $sv) {
                                           ...
                                           <div class="col">
                                             <input type="number" step="0.01"
                                                    class="form-control"
                                                    name="global_sauces[' . $si . '][price]"
                                                    value="' . s($sv['price']) . '"
                                                    placeholder="Price">
                                           </div>
                                           <div class="col-auto">
                                             <button type="button"
                                                     class="btn btn-danger remove-global-sauce">
                                               &times;
                                             </button>
                                           </div>
                                          </div>';
                                ...
                                <button type="button" class="btn btn-sm btn-primary mt-2" id="addGlobalSauce">
                                    Add Sauce
                                </button>
                            </div>
                        </div>
                    ...
                    <div id="sizesFields" class="mt-3"
                        style="display:<?= $has_sizes ? 'block' : 'none' ?>;">
                        <label><strong>Sizes</strong></label>
                        <small class="text-muted d-block">
                            Define sizes, base price, and how many sauces (max_sauces).
                        </small>
                        <div id="sizeBlocks">
                            <?php
                            if ($editing && isset($props['sizes']) && is_array($props['sizes'])) {
                                ...
                                <div class="row g-2 align-items-end">
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <input type="number"
                                               class="form-control"
                                               name="sizes_block[' . $sz_id . '][max_sauces]"
                                               value="' . $sz_maxSauce . '"
                                               min="0"
                                               placeholder="1">
                                    </div>
                                    <div class="col-md-2 text-end">
                                        <button type="button"
                                                class="btn btn-danger remove-size-block">
                                ...
                                <div class="row g-2 mt-3">
                                    <div class="col-md-6">
                                        <label>Extras</label>
                                        <select class="form-select extras-select"
                                                name="sizes_block[' . $sz_id . '][extras_selected][]"
                                                multiple></select>
                                    </div>
                                    <div class="col-md-6">
                                        <label>Sauces</label>
                                        <select class="form-select sauces-select"
                                                name="sizes_block[' . $sz_id . '][sauces_selected][]"
                                                multiple></select>
                                    </div>
                                </div>
                        ...
                        <button type="button" class="btn btn-primary btn-sm mt-2" id="addSizeBlock">
                            <i class="fas fa-plus-circle"></i> Add Size
                        </button>
                    </div>

                    <div id="dressesFields" class="mt-3"
                        style="display:<?= $has_dresses ? 'block' : 'none' ?>;">
                        <label><strong>Dresses</strong></label>
                        <small class="text-muted d-block">
                            Additional dress options.
                        </small>
                        <div id="dressBlocks">
                            <?php
                                ...
                                foreach ($props['dresses'] as $db_i => $db) {
                                    ...
                                    <div class="row g-2 align-items-end">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="number" step="0.01"
                                                   class="form-control"
                                                   name="dresses[' . $db_i . '][price]"
                                                   required
                                                   value="' . $dprice . '">
                                        </div>
                                        <div class="col-md-2 text-end">
                                            <button type="button"
                                                    class="btn btn-danger remove-dress-block">
                                              <i class="fas fa-minus-circle"></i>
                                            </button>
                                        </div>
                                    </div>
                        ...
                        <button type="button" class="btn btn-primary btn-sm mt-2" id="addDressBlock">
                            <i class="fas fa-plus-circle"></i> Add Dress
                        </button>
                    </div>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-check form-check-inline">
                        <input class="form-check-input"
                            type="radio" name="image_source"
                            value="upload" <?= $image_source === 'upload' ? 'checked' : '' ?>>
                        <label class="form-check-label">Upload</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input"
                            type="radio" name="image_source"
                            value="url" <?= $image_source === 'url' ? 'checked' : '' ?>>
                        <label class="form-check-label">URL</label>
                    </div>
                </div>
            </div>

            <div class="row g-3 mt-3" id="image_upload_field"
                style="display:<?= $img_u ?>;">
                <div class="col-md-6">
                    ...
                    <?php
                    if (
                        $editing && !empty($product['image_url'])
                        && strpos($product['image_url'], UPLOAD_DIR) === 0
                    ) {
                        echo '<div class="mt-2">
            ...
            <div class="row g-3 mt-3" id="image_url_field"
                style="display:<?= $img_ul ?>;">
                <div class="col-md-6">
    ...
    <?php
        ...
        echo '<a href="products.php?action=add"
        ...
        foreach ($products as $p) {
            ...
            if (!empty($pr['sizes'])) {
                foreach ($pr['sizes'] as $sz) {
                    echo '<strong>Size:</strong> '
                        . s($sz['size']) . ' - $'
                        . number_format($sz['price'], 2) . '<br>';
                    if (isset($sz['max_sauces'])) {
                        echo '&nbsp;&nbsp;<em>Max Sauces:</em> '
                            . (int)$sz['max_sauces']
                            . '<br>';
                    }
                    if (!empty($sz['extras'])) {
                ...
                echo '<strong>Base Price:</strong> $'
                    . number_format($pr['base_price'], 2)
                    . '<br>';
            }
            ...
            echo '</td>
              <td>';
            echo $p['is_offer']
                ? '<span class="badge bg-warning text-dark"><i class="fas fa-tags"></i></span>'
                : '-';
            echo '</td>
              <td>';
            echo $p['is_active']
                ? '<span class="badge bg-primary"><i class="fas fa-check-circle"></i></span>'
                : '<span class="badge bg-secondary"><i class="fas fa-times-circle"></i></span>';
            echo '</td>
        ...
        echo '<h2 class="mb-4">Delete Product</h2>'
            . $message
...
<script>
    $(document).ready(function() {
        ...
        <div class="row g-2 mb-2 global-extra">
          <div class="col">
            <input type="text" class="form-control"
                   name="global_extras[${c}][name]"
                   placeholder="Extra name">
          </div>
          <div class="col">
            <input type="number" step="0.01" class="form-control"
                   name="global_extras[${c}][price]"
                   placeholder="Price">
          </div>
          <div class="col-auto">
            <button type="button" class="btn btn-danger remove-global-extra">
              &times;
            </button>
          </div>
        </div>`);
        ...
        <div class="row g-2 mb-2 global-sauce">
          <div class="col">
            <input type="text" class="form-control"
                   name="global_sauces[${c}][name]"
                   placeholder="Sauce name">
          </div>
          <div class="col">
            <input type="number" step="0.01" class="form-control"
                   name="global_sauces[${c}][price]"
                   placeholder="Price">
          </div>
          <div class="col-auto">
            <button type="button" class="btn btn-danger remove-global-sauce">
              &times;
            </button>
          </div>
        </div>`);
            ...
            if (selectedExtras) {
                selectedExtras.forEach(function(extraName) {
                    ...
                    <div class="row g-2 mb-2 global-extra">
                      <div class="col">
                        <input type="text" class="form-control"
                               name="global_extras[${index}][name]"
                               value="${extraName}"
                               placeholder="Extra name">
                      </div>
                      <div class="col">
                        <input type="number" step="0.01" class="form-control"
                               name="global_extras[${index}][price]"
                               value="${price}"
                               placeholder="Price">
                      </div>
                      <div class="col-auto">
            ...
            if (selectedSauces) {
                selectedSauces.forEach(function(sauceName) {
                    ...
                    <div class="row g-2 mb-2 global-sauce">
                      <div class="col">
                        <input type="text" class="form-control"
                               name="global_sauces[${index}][name]"
                               value="${sauceName}"
                               placeholder="Sauce name">
                      </div>
                      <div class="col">
                        <input type="number" step="0.01" class="form-control"
                               name="global_sauces[${index}][price]"
                               value="${price}"
                               placeholder="Price">
                      </div>
                      <div class="col-auto">
                    ...
                    <div class="dress-block border p-3 mb-3">
                      <div class="row g-2 align-items-end">
                        <div class="col-md-6">
                          <label>Dress</label>
                          <input type="text" class="form-control"
                                 name="dresses[${index}][dress]"
                                 required
                                 value="${dressName}"
                                 placeholder="Italian Dressing">
                        </div>
                        <div class="col-md-4">
                          <label>Price($)</label>
                          <input type="number" step="0.01"
                                 class="form-control"
                                 name="dresses[${index}][price]"
                                 required
                                 value="${price}"
                                 placeholder="1.50">
                        </div>
                        <div class="col-md-2 text-end">
                          <button type="button" class="btn btn-danger remove-dress-block">
                            <i class="fas fa-minus-circle"></i>
                          </button>
                        </div>
                      </div>
      ...
      <div class="card-body" id="previewContent">No data yet...</div>
        ...
        $(document).on('input', 'input[name="image_url"]', function() {
            ...
            if (url !== '') {
              ...
              <img src="${url}"
                   onload="this.style.maxWidth='200px'"
                   onerror="$('#image_url_preview').html('<small class=\'text-danger\'>Invalid image URL</small>')" />
            `);
            } else {
                    ...
                    if (sz.trim() !== '') {
                        ...
                        if (extrasSelected.length > 0) {
                            ...
                            $.each(extrasSelected, function(_, exIndex) {
                                exIndex = parseInt(exIndex, 10);
                                if (globalExtras[exIndex]) {
                                    var exPrice = $thisBlock.find('input[name*="[extras_prices][' + exIndex + ']"]').val() ||
                                        globalExtras[exIndex].price || 0;
                        ...
                        if (saucesSelected.length > 0) {
                            ...
                            $.each(saucesSelected, function(_, saIndex) {
                                saIndex = parseInt(saIndex, 10);
                                if (globalSauces[saIndex]) {
                                    var saPrice = $thisBlock.find('input[name*="[sauces_prices][' + saIndex + ']"]').val() ||
                                        globalSauces[saIndex].price || 0;
        ...
        <div class="size-block border p-3 mb-3" data-block-index="${index}">
          <div class="row g-2 align-items-end">
            <div class="col-md-4">
              ...
              <input type="text"
                     class="form-control"
                     name="sizes_block[${index}][size]"
                     required
                     placeholder="Medium / 10 pieces...">
            </div>
            <div class="col-md-3">
              <label>Base Price($)</label>
              <input type="number"
            ...
            <div class="col-md-3">
              ...
              <input type="number"
                     class="form-control"
                     name="sizes_block[${index}][max_sauces]"
                     min="0"
                     placeholder="1">
            </div>
            <div class="col-md-2 text-end">
              <button type="button" class="btn btn-danger remove-size-block">
                <i class="fas fa-minus-circle"></i>
              </button>
            </div>
          </div>
          <div class="row g-2 mt-3">
            <div class="col-md-6">
              <label>Extras</label>
              <select class="form-select extras-select"
                      name="sizes_block[${index}][extras_selected][]"
                      multiple></select>
            </div>
            <div class="col-md-6">
              <label>Sauces</label>
              <select class="form-select sauces-select"
                      name="sizes_block[${index}][sauces_selected][]"
                      multiple></select>
            </div>
          </div>
          <div class="extras-prices mt-3"></div>
