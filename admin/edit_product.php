...
// Fetch sizes and prices if category is Pizza
$selected_category = fetchAll($pdo, 'SELECT name FROM categories WHERE id = ?', [$product['category_id']]);
$is_pizza = false;
...
$prices = [];
if ($is_pizza) {
    $product_sizes = fetchAll($pdo, 'SELECT * FROM product_sizes WHERE product_id = ?', [$id]);
...
$mixes = fetchAll($pdo, 'SELECT * FROM products WHERE id != ? ORDER BY name ASC', [$id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($product_code) || empty($name) || $price <= 0 || $category_id === 0) {
        ...
        $sql_check = 'SELECT COUNT(*) FROM products WHERE product_code = ? AND id != ?';
        $params_check = [$product_code, $id];
        ...
        if ($stmt_check->fetchColumn() > 0) {
            ...
            if ($image_source === 'upload') {
                if (isset($_FILES['image_file']) && $_FILES['image_file']['error'] !== UPLOAD_ERR_NO_FILE) {
                    $upload = handleImageUpload($_FILES['image_file'], true, $product['image_url']);
                    if ($upload['success']) {
                ...
                if ($validate['valid']) {
                    ...
                    // Keep existing image_url if validation fails and there's already an image_url
            ...
            if (
                ...
                (!$is_pizza) || // Non-Pizza categories
                ($is_pizza && empty($sizes) && empty($prices)) // Pizza with no sizes and prices
            ) {
                ...
                try {
                    ...
                    if (!empty($selected_sauces)) {
                        ...
                        foreach ($selected_sauces as $sauce_id) {
                            $stmt_sauces->execute([$id, $sauce_id]);
                        }
                    }
                    ...
                    if (!empty($selected_mixes)) {
                        ...
                        foreach ($selected_mixes as $mixed_product_id) {
                            $stmt_mixes->execute([$id, $mixed_product_id]);
                        }
                    }
                        ...
                        if (!empty($sizes) && !empty($prices)) {
                            ...
                            foreach ($sizes as $index => $size) {
                                ...
                                if (!empty($size) && $price_size !== false && $price_size >= 0) {
                                    $stmt_size->execute([$id, $size, $price_size]);
                                }
                            }
                        }
                    }
...
<form method="POST" action="edit_product.php?id=<?= $id ?>" enctype="multipart/form-data">
    <div class="row g-3">
        ...
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control" id="product_code" name="product_code" required value="<?= sanitizeInput($_POST['product_code'] ?? $product['product_code']) ?>">
            </div>
        </div>
        <!-- Product Name -->
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control" id="name" name="name" required value="<?= sanitizeInput($_POST['name'] ?? $product['name']) ?>">
            </div>
        </div>
    </div>
    ...
    <div class="row g-3 mt-3">
            ...
            <div class="input-group">
                ...
                <select class="form-select" id="category_id" name="category_id" required>
                    ...
                    <?php foreach ($categories as $category): ?>
                        <option value="<?= $category['id'] ?>" <?= (($product['category_id'] == $category['id']) || (isset($_POST['category_id']) && $_POST['category_id'] == $category['id'])) ? 'selected' : '' ?>>
        ...
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <input type="number" step="0.01" class="form-control" id="price" name="price" required value="<?= sanitizeInput($_POST['price'] ?? $product['price']) ?>">
            </div>
        </div>
    </div>
    ...
    <?php if ($is_pizza): ?>
        <div class="row g-3 mt-3" id="sizes_prices_section" style="display: block;">
            <div class="col-md-12">
                ...
                <table class="table table-bordered" id="sizesPricesTable">
                    ...
                    <tbody>
                        ...
                        if (isset($_POST['sizes']) && is_array($_POST['sizes'])) {
                            ...
                            // Populate existing sizes and prices
                            if (!empty($sizes) && !empty($prices)) {
                                foreach ($sizes as $index => $size) {
                                    $price = $prices[$index];
                                    echo '<tr>
    ...
    <?php else: ?>
        <div class="row g-3 mt-3" id="sizes_prices_section" style="display: none;"></div>
    <?php endif; ?>
    ...
    <div class="row g-3 mt-3">
        <div class="col-md-12">
            ...
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="image_source" id="image_source_upload" value="upload" <?= ((!isset($_POST['image_source']) && strpos($product['image_url'], UPLOAD_DIR) === 0) || (isset($_POST['image_source']) && $_POST['image_source'] === 'upload')) ? 'checked' : '' ?>>
                <label class="form-check-label" for="image_source_upload">Upload Image</label>
            </div>
            <div class="form-check form-check-inline">
                <input class="form-check-input" type="radio" name="image_source" id="image_source_url" value="url" <?= ((isset($_POST['image_source']) && $_POST['image_source'] === 'url') || (!empty($product['image_url']) && strpos($product['image_url'], UPLOAD_DIR) !== 0)) ? 'checked' : '' ?>>
    ...
    <!-- Image Upload Field -->
    <div class="row g-3 mt-3" id="image_upload_field" style="display: <?= ((!isset($_POST['image_source']) && strpos($product['image_url'], UPLOAD_DIR) === 0) || (isset($_POST['image_source']) && $_POST['image_source'] === 'upload')) ? 'block' : 'none' ?>;">
        <div class="col-md-6">
            ...
            <?php if (!empty($product['image_url']) && strpos($product['image_url'], UPLOAD_DIR) === 0): ?>
    ...
    <!-- Image URL Field -->
    <div class="row g-3 mt-3" id="image_url_field" style="display: <?= ((isset($_POST['image_source']) && $_POST['image_source'] === 'url') || (!empty($product['image_url']) && strpos($product['image_url'], UPLOAD_DIR) !== 0)) ? 'block' : 'none' ?>;">
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <input type="url" class="form-control" id="image_url" name="image_url" placeholder="https://example.com/image.jpg" value="<?= sanitizeInput($_POST['image_url'] ?? $product['image_url']) ?>">
            </div>
            <?php if (!empty($product['image_url']) && strpos($product['image_url'], UPLOAD_DIR) !== 0): ?>
                <div class="mt-2"><img src="<?= sanitizeInput($product['image_url']) ?>" alt="Product Image" style="max-width: 200px;"></div>
            <?php endif; ?>
    ...
    <div class="row g-3 mt-3">
        ...
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <input type="text" class="form-control" id="allergies" name="allergies" placeholder="e.g., Nuts, Milk" value="<?= sanitizeInput($_POST['allergies'] ?? $product['allergies']) ?>">
            </div>
        ...
        <div class="col-md-6">
            ...
            <div class="input-group">
                ...
                <textarea class="form-control" id="description" name="description"><?= sanitizeInput($_POST['description'] ?? $product['description']) ?></textarea>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-4">
            ...
            <select class="form-select" id="extras" name="extras[]" multiple>
                <?php foreach ($extras as $extra): ?>
                    <option value="<?= $extra['id'] ?>" <?= (isset($_POST['extras']) && in_array($extra['id'], $_POST['extras'])) || in_array($extra['id'], $selected_extras) ? 'selected' : '' ?>>
        ...
        <div class="col-md-4">
            ...
            <select class="form-select" id="sauces" name="sauces[]" multiple>
                <?php foreach ($sauces as $sauce): ?>
                    <option value="<?= $sauce['id'] ?>" <?= (isset($_POST['sauces']) && in_array($sauce['id'], $_POST['sauces'])) || in_array($sauce['id'], $selected_sauces) ? 'selected' : '' ?>>
        ...
        <div class="col-md-4">
            ...
            <select class="form-select" id="mixes" name="mixes[]" multiple>
                <?php foreach ($mixes as $mix_product): ?>
                    <option value="<?= $mix_product['id'] ?>" <?= (isset($_POST['mixes']) && in_array($mix_product['id'], $_POST['mixes'])) || in_array($mix_product['id'], $selected_mixes) ? 'selected' : '' ?>>
    ...
    <div class="row g-3 mt-3">
        ...
        <div class="col-md-4">
            <div class="form-check">
                <input type="checkbox" class="form-check-input" id="is_new" name="is_new" <?= (isset($_POST['is_new']) && $_POST['is_new']) ? 'checked' : ($product['is_new'] ? 'checked' : '') ?>>
        ...
        <div class="col-md-4">
            <div class="form-check">
                <input type="checkbox" class="form-check-input" id="is_offer" name="is_offer" <?= (isset($_POST['is_offer']) && $_POST['is_offer']) ? 'checked' : ($product['is_offer'] ? 'checked' : '') ?>>
    ...
    <!-- Form Buttons -->
    <div class="mt-4">
        <button type="submit" class="btn btn-warning me-2" data-bs-toggle="tooltip" title="Update product">
